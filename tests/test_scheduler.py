...
class TestSchedulerCheck(QdsCliTestCase):
    ...
    def test_edit_name(self):
        sys.argv = ['qds.py', 'scheduler', 'edit', '123', "--name", "new_name"]
        print_command()
        Connection._api_call = Mock()
        Connection._api_call.side_effect = list_actions_side_effect
        qds.main()
        Connection._api_call.assert_called_with("PUT", "scheduler/123", {'name': 'new_name'})

    def test_edit_name_concurrency(self):
        sys.argv = ['qds.py', 'scheduler', 'edit', '123', "--name", "new_name", "--concurrency", "5"]
        print_command()
        Connection._api_call = Mock()
        Connection._api_call.side_effect = list_actions_side_effect
        qds.main()
        Connection._api_call.assert_called_with("PUT", "scheduler/123", {'name': 'new_name', 'concurrency': '5'})
